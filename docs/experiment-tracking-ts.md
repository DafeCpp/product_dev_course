...
- **Язык и HTTP-слой:** Python 3.14+, фреймворк `aiohttp` (сервер и клиентский доступ), middlewares и роутинг строятся на стандартных паттернах курса.
...
- **Многоуровневый доступ к проектам:** к проекту могут иметь доступ несколько пользователей с разными ролями. При запросе списка проектов (эндпоинт `GET /projects`) пользователь получает все доступные ему проекты — те, где он является участником через таблицу `project_members`, независимо от роли (owner/editor/viewer).
- Поиск и фильтрация проектов через UI.
