...
- **Передает `trace_id` и `request_id` в заголовках при проксировании запросов**
...
### 11.3. Общие требования для всех сервисов

**Важно:** Требования по `trace_id` и `request_id` применяются ко **всем сервисам** в системе:
- **Auth Service** (текущий документ)
- **Experiment Service**
- **Auth Proxy (BFF)**
- **API Gateway** (если используется)
- **Любые другие микросервисы**

Все сервисы должны:
1. Генерировать `request_id` для каждого входящего HTTP запроса
2. Извлекать или генерировать `trace_id` из заголовка `X-Trace-Id`
3. Передавать `trace_id` в заголовке `X-Trace-Id` при вызовах других сервисов
4. Включать `trace_id` и `request_id` во все логи
5. Использовать структурированное логирование (JSON) для удобного поиска и анализа
