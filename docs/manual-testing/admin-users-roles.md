...
**Факт (2026-07-16):** ✅ Шаг 1: пользователь `qa_viewer` (без permissions) видит в nav только Проекты/Эксперименты/Датчики/Монитор/Телеметрия/Webhooks — Пользователи/Аудит/Скрипты/Конфиги/Rate Limits корректно скрыты. ✅ Шаг 2: прямой заход на `/admin/users` — роут открывается (нет route-level гейта), но `GET /auth/admin/users` и `/auth/admin/invites` → `403 Forbidden`, показаны error-тосты «Request failed with status code 403»; данные не отдаются. Шаг 3 не прогонялся отдельно.
