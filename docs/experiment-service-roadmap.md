...
- **✅ Завершено (Foundation):** блок Foundation полностью (миграции, CRUD для `Experiment/Run/CaptureSession`, idempotency, пагинация, OpenAPI, RBAC). Добавлены домены `Sensor` и `ConversionProfile`, статусные машины и покрытие тестами (`tests/test_api_*`). Множественные проекты для датчиков реализованы на backend (миграция `002_sensor_projects_many_to_many.sql`, API endpoints, тесты). UI для управления доступом к проектам реализован (`ProjectMembersModal` с тестами).
...
- **⚠️ Частично реализовано (Frontend):** UI для управления участниками проектов реализован (`ProjectMembersModal`). Профиль пользователя реализован (`UserProfileModal`). Фильтрация экспериментов по проектам с выпадающим списком реализована (`ExperimentsList.tsx`). Фильтрация датчиков по проектам требует доработки (текстовое поле вместо выпадающего списка в `SensorsList.tsx`). UI для управления множественными проектами датчиков отсутствует. Модальное окно для редактирования проектов отсутствует (есть только создание).
...
- **Множественные проекты для датчиков:** ⚠️ Частично реализовано (backend готов, frontend отсутствует).
  ...
  - ❌ **Frontend:** Не реализовано. Отсутствует UI для управления множественными проектами датчиков: нет методов в API клиенте (`sensorsApi.addProject`, `sensorsApi.removeProject`, `sensorsApi.getProjects`), нет UI компонентов для отображения списка проектов датчика и управления связями (добавление/удаление проектов). В `SensorDetail.tsx` отображается только один `project_id`, в `SensorsList.tsx` нет функциональности для управления проектами датчика. Требуется реализация UI компонентов и интеграция с backend API.
- **Фильтрация датчиков по доступным проектам:** ⚠️ Частично реализовано (backend готов, frontend требует доработки).
  ...
  - ⚠️ **Frontend:** Частично реализовано. В `SensorsList.tsx` есть фильтр по `project_id` (текстовое поле для ввода UUID), но отсутствует выпадающий список доступных проектов. Нет автоматического отображения всех датчиков из доступных проектов при отсутствии выбранного проекта. Требуется доработка UI для использования выпадающего списка проектов (аналогично `ExperimentsList.tsx`) и автоматического выбора первого доступного проекта.
...
- **Модальное окно для просмотра и редактирования проектов:** ❌ Требуется реализация. Модальное окно для просмотра и редактирования информации о проекте (название, описание, метаданные). Редактирование доступно только для владельцев и редакторов проекта. Бэкенд API готов в `auth-service/api/routes/projects.py` (endpoints: `GET /projects/{id}`, `PATCH /projects/{id}`).
