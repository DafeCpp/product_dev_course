...
- **❌ Не реализовано: нормальный вывод ошибок для дебага (frontend + BFF, toast снизу справа):** при ошибках показывать **детали запроса/ответа** и **trace-id/request-id**, чтобы сразу можно было сопоставить UI ↔ логи/трейсы.
  ...
  - **Ответ:** HTTP status + сырое тело ответа (если есть).
...
- **SSE stream UI (MVP):** ✅ Реализовано во фронтенде: `TelemetryStreamModal` (подключение через `fetch` + ReadableStream, т.к. требуется `Authorization: Bearer ...`), кнопка **Live telemetry** в карточке датчика (`SensorDetail.tsx`/`SensorDetailModal.tsx`), покрыто тестом `TelemetryStreamModal.test.tsx`.
