...
Все сервисы настроены для вывода JSON логов с полями `trace_id`, `request_id`, `level`, и `path`. Promtail автоматически извлекает эти поля как labels для удобной фильтрации.
...
Promtail автоматически извлекает следующие поля как labels:
...
1. **Labels vs Fields**: `trace_id`, `request_id`, `level`, и `path` извлекаются как **labels** в Promtail, что позволяет быстро фильтровать по ним.
