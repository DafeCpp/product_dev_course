# Техническое задание (ТЗ): `telemetry-cli`

Версия: 0.1
Дата: 2025-12-25
Проект: `projects/telemetry_cli`
...
- **A1**: Запуск `telemetry-cli --config <yaml>` стартует агент, который валидирует конфиг и начинает генерацию/приём readings.
- **A2**: При `synthetic` агент формирует readings согласно `signals[]` и отправляет их батчами, соблюдая правила из п. 9.
- **A3**: При `udp_json` агент корректно принимает обе формы payload (single/multi), корректно парсит `ts_ms`/`timestamp`, и игнорирует мусорные датаграммы без падения процесса.
- **A4**: При `esp32_ws` агент принимает сообщения `type="telem"`, корректно маппит поля в сигналы, и переподключается при разрыве.
- **A5**: Запрос в Experiment Service соответствует формату из п. 8 (включая `Authorization: Bearer ...` и `readings[].meta.signal`).
