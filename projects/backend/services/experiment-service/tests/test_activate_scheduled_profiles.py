...
def _make_pool_mock(activated_ids: list[uuid.UUID], archived_ids: list[uuid.UUID]) -> AsyncMock:
    """Build a pool mock whose conn.fetch returns the given UUID lists in order."""
    activated_rows = [{"id": uid} for uid in activated_ids]
    archived_rows = [{"id": uid} for uid in archived_ids]

    conn = AsyncMock()
    conn.fetch = AsyncMock(side_effect=[activated_rows, archived_rows])
...
async def test_activate_scheduled_profiles_activates_due_drafts() -> None:
    ...
    pool = _make_pool_mock(activated_ids=[profile_id], archived_ids=[])
...
@pytest.mark.asyncio
async def test_activate_scheduled_profiles_archives_expired() -> None:
    """active profiles with valid_to < now must become archived."""
    profile_id = uuid.uuid4()
    pool = _make_pool_mock(activated_ids=[], archived_ids=[profile_id])
...
@pytest.mark.asyncio
async def test_activate_scheduled_profiles_both_activated_and_archived() -> None:
    """When both activations and archivations happen, both counts appear in summary."""
    pool = _make_pool_mock(
        activated_ids=[uuid.uuid4(), uuid.uuid4()],
        archived_ids=[uuid.uuid4()],
    )
...
async def test_activate_scheduled_profiles_returns_none_when_nothing() -> None:
    ...
    pool = _make_pool_mock(activated_ids=[], archived_ids=[])
