...
### 1. Аутентификация (Auth) 🔴 КРИТИЧНО
**Требование MVP:** Auth должен работать

**Что отсутствует:**
- ❌ Страница входа (`/login`)
- ❌ Страница регистрации (если нужна)
- ❌ Интеграция с Auth Proxy (BFF)
- ❌ Обработка refresh токенов
- ❌ Страница профиля пользователя (`/auth/me`)

**Что нужно:**
1. Создать страницу `/login` с формой входа
2. Интегрировать с `/auth/login` endpoint (через Auth Proxy)
3. Сохранение токенов (сейчас только access_token в localStorage, нужно через куки от Auth Proxy)
4. Обработка refresh токенов
5. Защита роутов (redirect на login если не авторизован)
...
### 2. Управление датчиками (Sensors) 🔴 КРИТИЧНО
**Требование MVP:** Список датчиков

**Что отсутствует:**
- ❌ Страница списка датчиков
- ❌ Страница создания/регистрации датчика
...
2. Добавить API методы в `src/api/client.ts`:
   - `sensorsApi.list(projectId)`
   - `sensorsApi.get(id)`
   - `sensorsApi.create(data)`
   - `sensorsApi.update(id, data)`
   - `sensorsApi.delete(id)`
   - `sensorsApi.rotateToken(id)`

3. Создать страницы:
   - `/sensors` - список датчиков
   - `/sensors/new` - регистрация нового датчика
   - `/sensors/:id` - детали датчика

4. Компоненты:
   - `SensorsList` - список с фильтрацией
   - `SensorForm` - форма создания/редактирования
   - `SensorDetail` - детальный просмотр
...
3. ❌ **Аутентификация** - нужно сделать
4. ❌ **Список датчиков** - нужно сделать
