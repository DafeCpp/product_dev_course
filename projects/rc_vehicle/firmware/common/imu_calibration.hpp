...
struct ImuCalibData {
  ...
  /**
   * Единичный СЫРОЙ вектор показания акселерометра в покое в СК датчика
   * (этап 1: стояние на месте). Это опора ориентации Madgwick для сырых
   * accel-семплов; не заменять на RestDownVec()/пост-Apply кадр.
   */
  float gravity_vec[3]{0.f, 0.f, 1.f};
  /**
   * Единичный вектор «вперёд» в СК датчика, выученный по линейному ускорению
   * после Apply(). Хранится как физическое направление оси машины и поэтому
   * ортогонален сырому gravity_vec, но GetForwardAccel() применяет его к
   * bias-скорректированным данным после вычитания RestDownVec().
   *
   * Важно не путать две опоры: gravity_vec остаётся сырым вектором покоя для
   * Madgwick, а RestDownVec() — вертикаль данных после Apply() для выделения
   * линейного ускорения. SetVehicleFrame() дополнительно проецирует forward на
   * плоскость, перпендикулярную gravity_vec.
   */
...
class ImuCalibration {
 public:
   ...
   *
   * Считается как скалярное произведение ((accel после Apply) − RestDownVec())
   * на accel_forward_vec. Гравитацию в пост-Apply кадре обязательно вычитаем,
   * иначе остаточный наклон оси «вперёд» даёт постоянный офсет (LOS-214). В
   * покое на ровной площадке возвращает ~0.
   */
  float GetForwardAccel(const ImuData& data) const;

  /** Задать направление «вперёд» единичным вектором в СК датчика (fx,fy,fz).
   * Нормализуется и приводится к горизонтали (⊥ gravity_vec) для хранения в
   * accel_forward_vec. */
   ...
   *
   * @param data  Откалиброванные IMU-данные (после Apply). Модифицируется
   * in-place.
