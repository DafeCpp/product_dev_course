...
struct KidsModeConfig {
  ...
  /**
   * Скорость адаптации потолка speed limiter'а (LOS-285), используется
   * только при выключенной мотор-модели (filter.motor_model_enabled ==
   * false): на сколько в секунду сдвигается адаптивный потолок газа на
   * каждый м/с ошибки EKF-скорости относительно max_speed_ms. При включённой
   * мотор-модели (по умолчанию) ограничение целиком детерминированное — сам
   * потолок вычисляется из motor_speed_gain, и это поле не используется.
   * Диапазон: [0.5..10.0].
   */
