...
/**
 * @brief Результат ПРИЁМА команды erase — не результат самого стирания.
 *
 * target_seq назначается АТОМАРНО в момент постановки в очередь (та же
 * критическая секция, что и сам факт приёма), а не читается отдельным
 * вызовом GetMagCalibState() после EraseMagCalibration(): между этими двумя
 * вызовами control loop успевает вклиниться, забрать запрос из очереди и
 * применить его — тогда снимок уже показывал бы erase_seq ЭТОЙ команды, а
 * target, посчитанный от него, указывал бы на следующий erase, которого не
 * будет (ревью PR #308). target_seq — это порядковый номер СРЕДИ ERASE (не
 * среди всех команд), и он всегда совпадает с тем erase_seq, который
 * опубликует control loop, когда применит именно эту, и только эту,
 * команду — потому что оба счётчика растут на 1 на каждый erase, в одном и
 * том же FIFO-порядке.
 */
struct MagCalibEraseAck {
  bool accepted{false};
...
class IVehicleControl {
 public:
   ...
   *
   * Возвращает MagCalibEraseAck, а не bool: target_seq клиенту нужен для
   * поллинга результата ИМЕННО этой команды, и назначать его нужно в тот же
   * момент, когда решается сам факт приёма — иначе гонка с control loop
   * (ревью PR #308).
   */
  virtual MagCalibEraseAck EraseMagCalibration() = 0;
