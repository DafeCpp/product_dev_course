...
- **context.hpp** — контекст с регистрацией компонентов по типу (compile-time): `Context<UartBridgeBase, SpiBase, ...>` — список типов задаётся при объявлении; `Set<T>(ref)` регистрирует по ссылке, `Get<T>()` возвращает компонент по типу T; опционально `get_time_ms`.
...
- **spi_base.hpp** — абстрактный базовый класс SPI-драйвера:
  - чисто виртуальные: `Init()`, `Transfer(tx, rx)` (полнодуплексный обмен; `tx.size()==rx.size()`; реализация держит CS на время обмена).
- **mpu6050_spi.hpp / mpu6050_spi.cpp** — драйвер MPU-6050 по SPI: структура `ImuData`, класс `Mpu6050Spi(SpiBase*)` с `Init()`, `Read(ImuData&)`, `ConvertToTelem(const ImuData&, int16_t&, ...)`.
