...
void MadgwickFilter::SetVehicleFrame(const float gravity_vec[3],
  ...
  // Три источника по убыванию приоритета: перенос сошедшегося курса через
  // смену базиса, засев из магнитометра (9DOF), принудительный ноль.
  float yaw_rad = 0.f;

  if (preserve_yaw) {
    // 1. Курс относительно НОВОГО монтажа: q_madgwick_prev (снятый до
    ...
    //
    // Приоритет выше засева (ревью PR #308). Оба значения — одна и та же
    // точка равновесия MARG, но получены по-разному: сошедшийся курс собран
    // градиентным спуском за секунды поля и потому усреднён, а засев берёт
    // ОДИН мгновенный семпл. Смена СК может произойти и на ходу —
    // CalibrationManager::ProcessForwardDirectionRequest() (WS-команда
    // set_forward_direction) проверяет только gravity_valid и не требует
    // остановки, — а на ходу поле искажают токи мотора (в логах LOS-221 |m|
    // гуляет 485…770 мГс). Засев по такому семплу дал бы скачок курса и
    // новую рампу сходимости — ровно то, что этот фикс и убирает.
    float qw, qx, qy, qz;
  ...
  } else if (SeedYawFromMag(fx, fy, fz, yx, yy, yz, yaw_rad)) {
    // 2. Аналитический засев из магнитометра (LOS-221) — сразу ТОЧКА
    // РАВНОВЕСИЯ MARG, поэтому градиентному спуску нечего догонять и фантомной
    // рампы курса не возникает вовсе.
    //
    // Сюда попадаем именно тогда, когда сохранять нечего: это либо ПЕРВАЯ
    // калибровка (use_vehicle_frame_ == false), либо курс не подкреплён
    // магнитометром. Оба условия выполняются на загрузочной калибровке — том
    // самом случае, ради которого засев и сделан.
    //
