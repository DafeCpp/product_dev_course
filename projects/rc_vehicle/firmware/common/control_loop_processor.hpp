...
class ControlLoopProcessor {
 ...
 private:
  ...
  // true, когда tilt_comp_enabled был активен на ПРЕДЫДУЩЕМ тике — ловит
  // переход выключено→включено в рантайме (код-ревью PR #290, 9-й раунд):
  // пока tilt_comp_enabled=false, tilt_est_.Update() не вызывается вовсе, а
  // её внутренние pitch_rad_/roll_rad_ остаются ЗАМОРОЖЕНЫ на последнем
  // значении. Без сброса при повторном включении EKF на первом же тике
  // получил бы протухший тангаж/крен из интервала, когда фильтр был
  // выключен — до нескольких секунд ложной grav-компенсации при
  // corr_gain_hz по умолчанию (аналогично сбросу на смену СК от
  // калибровки, 5-й раунд, но здесь триггер — сам флаг конфига).
