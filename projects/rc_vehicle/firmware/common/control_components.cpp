...
void ImuHandler::Update(uint32_t now_ms, [[maybe_unused]] uint32_t dt_ms) {
  ...
  // LOS-222: приводим опубликованную и используемую в динамике IMU СК к
  // правой тройке автомобиля: X смотрит вперёд, положительный yaw_rate —
  // левый поворот (согласованно с ростом yaw_deg). Сырые accel до этой
  // коррекции уже сохранены выше и продолжают кормить Madgwick, потому что
  // фильтр ориентации работает в СК датчика/калибровки.
  data_.ax = -data_.ax;
  data_.gz = -data_.gz;
