 ...
 *
 * События СТАРШЕ первого кадра отбрасываются: кольцо кадров (и кольцо
 * событий) переполняется независимо, а «Очистить лог» до этой правки чистил
 * только кадры, — поэтому в буфере легко оказываются события прошлых
 * прогонов. Прижимать их к нулевому кадру нельзя: свежий CSV начинался бы
 * с чужого TestStart.
 *
...
function assignEventsToFrames(events, frameTs) {
    ...
    // Всё, что старше окна кадров, — из прошлых прогонов
    const sorted = events.filter(ev => ev.ts >= frameTs[0])
                         .sort((a, b) => a.ts - b.ts);
    let frameIdx = 0;
