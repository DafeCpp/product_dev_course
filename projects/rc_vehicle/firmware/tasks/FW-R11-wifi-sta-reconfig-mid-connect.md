...
- [ ] Железо: смена пароля из Web UI применяется **без перезагрузки** —
      STA подключается к новой сети сразу после «Подключить»
- [ ] Железо: при вводе верного пароля поверх неверного из NVS подключение
      проходит с первой попытки, без спама «sta is connecting»
