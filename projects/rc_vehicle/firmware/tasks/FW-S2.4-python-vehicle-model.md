...
**Приоритет:** MEDIUM
**Статус:** [x] Готово
**Файлы:** `firmware/sim/simlib/` (`sim_params.py`, `vehicle_model.py`,
`sensors.py`, `frame.py`), `firmware/sim/tests/` (pytest), `firmware/sim/pyproject.toml`.
...
- [x] Модель выдаёт сырьё (IMU/mag) в формате кадра протокола (`SensorFrame.to_csv`
  совпадает с `ParseInputLine` из FW-S2.1, 17 полей).
- [x] pytest (13 тестов) на здравые свойства: руль → знак yaw rate; задний ход →
  обратный yaw; газ → разгон с τ; мёртвая зона; наклон IMU → проекция g в accel;
  `gz` = yaw rate; лимиты/slew серво; сохранение модуля mag-вектора.
- [x] `SimParams` параметризованы и документированы (единицы, источник значений);
  динамический велосипед (Caf/Car) реализован опцией.

**Реализация:** `firmware/sim/` (пакет `simlib`, numpy). Кинематический +
динамический велосипед, мотор первого порядка (τ + breakaway), серво (лимит+slew),
синтез accel/gyro/mag. Запуск: `pip install -e ".[dev]" && python -m pytest`.
