...
TEST(PidTest, IOnlyAccumulates) {
  ...
  (void)pid.Step(error, dt);  // integral = 10 * 0.002 = 0.02
  EXPECT_NEAR(pid.GetIntegral(), error * dt, 1e-5f);

  (void)pid.Step(error, dt);  // integral = 0.04
...
TEST(PidTest, DOnlySecondStepIsCorrect) {
  PidController pid({.kp = 0.0f,
                     ...
                     .max_output = 1000.0f});
  const float dt = 0.01f;
  (void)pid.Step(2.0f, dt);              // first step, D = 0
...
TEST(PidTest, ResetMakesNextStepLikeFirstStep) {
  PidController pid({.kp = 0.0f,
                     ...
                     .max_integral = 1.0f,
                     .max_output = 100.0f});
  (void)pid.Step(2.0f, 0.01f);
  (void)pid.Step(4.0f, 0.01f);  // now first_step_ = false
