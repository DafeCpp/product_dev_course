...
using firmware_common::ApplySlewRate;
...
TEST(SlewRate, NoChange_WhenDtZero) {
  ...
  float result = ApplySlewRate(1.0f, 0.0f, 1.0f, 0.0f);
  EXPECT_FLOAT_EQ(result, 0.0f);
}

TEST(SlewRate, NoChange_WhenMaxRateZero) {
  // max_change_per_sec=0 → always stays at current
  float result = ApplySlewRate(1.0f, 0.5f, 0.0f, 0.1f);
  EXPECT_FLOAT_EQ(result, 0.5f);
}
...
TEST(SlewRate, ConvergesOverTime) {
  ...
  float val = 0.0f;
  for (int i = 0; i < 5; ++i) {
    val = ApplySlewRate(1.0f, val, 2.0f, 0.1f);
  }
