...
| R4 | **LOW** | ~~**«Защита от переполнения» в Failsafe срабатывает наоборот.»**~~ — **WONTFIX (решение 2026-06-11)**: проверка `last_active_ms_ > now_ms` оставлена. Беззнаковое вычитание само корректно обрабатывает wrap uint32 (~49.7 суток аптайма), и проверка даёт ложный failsafe в момент переполнения — но это ошибка в безопасную сторону (машина остановится), компромисс принят осознанно. Детали: `tasks/FW-R4-failsafe-wraparound-check.md` | `common/failsafe.cpp` | 37-38, 67 |
