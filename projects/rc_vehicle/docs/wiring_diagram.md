...
**Подключение:**
- `Input` → Батарея 2S LiPo (7.4V) напрямую
- `Output 5V` →
  - ESP32-S3 (пин 5V на плате → встроенный LDO → 3.3V для чипа)
  - Servo (+)
  - RC приёмник (если требуется)
- `GND` → общая земля всех компонентов

**Конденсаторы для стабилизации питания (обязательно!):**

⚠️ **Проблема:** при резком газе с места ESC берёт пиковый ток из батареи → напряжение на входе UBEC проседает → на выходе UBEC проседает 5V → LDO на плате ESP32 не держит 3.3V → срабатывает brownout detector → **контроллер перезагружается**.

Схема фильтрации (три точки):

```
Батарея 2S ──[C1]── UBEC ──[C2]── 5Vin ESP32 (→ LDO на плате → 3.3V)
```

| Точка | Конденсатор | Номинал | Напряжение | Назначение |
|-------|-------------|---------|------------|------------|
| C1 — перед UBEC (вход) | Электролит | 1000–2200 мкФ | ≥16V | Сглаживает просадки батареи при пиковом токе мотора |
| C2 — после UBEC (выход 5V) | Электролит + керамика | 470–1000 мкФ + 100 нФ | ≥6.3V | Компенсирует медленную реакцию UBEC; керамика давит ВЧ шум |
| C3 — на 3.3V (опционально) | Электролит + керамика | 100–220 мкФ + 100 нФ | ≥6.3V | Локальный запас энергии у чипа (паять к пинам 3V3/GND на плате) |

**Рекомендация:** начать с C1 + C2. Если перезагрузки продолжаются — добавить C3 на 3.3V и/или понизить порог brownout detector в sdkconfig (ESP-IDF: `CONFIG_ESP_BROWNOUT_DET_LVL`).
