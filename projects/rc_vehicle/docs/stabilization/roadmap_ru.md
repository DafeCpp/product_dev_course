...
**Задачи**:
- [ ] Обеспечить стабильное чтение IMU (MPU-6500, SPI) на частоте ≥500 Гц на Core 1
- [ ] Реализовать калибровку IMU:
  - [ ] Ручная калибровка: сбор offset/bias гироскопа и акселерометра в покое по команде оператора
  - [ ] Автоматическая калибровка гироскопа при старте (auto-zero bias при неподвижности)
  - [ ] Автоматическая калибровка акселерометра (компенсация offset по среднему значению g-вектора)
  - [ ] Сохранение калибровочных данных в NVS (переживают перезагрузку)
  - [ ] Валидация калибровки: детекция устаревших/невалидных данных (температурный дрейф, замена датчика)
  - [ ] WebSocket API: запуск калибровки, просмотр статуса и текущих значений
- [ ] Реализовать фильтр Madgwick (AHRS) для оценки ориентации (кватернион → pitch/roll/yaw)
- [ ] Реализовать LPF Butterworth 2-го порядка для фильтрации gyro Z (подготовка к yaw rate PID)
- [ ] Добавить конфигурационные параметры стабилизации (через WebSocket API)
- [ ] Создать режим "стабилизация выключена" (прямая передача команд)
- [ ] Добавить данные фильтрации в телеметрию (ориентация, фильтрованный gyro, статус калибровки)
- [ ] Организовать control loop на Core 1 (высокоприоритетный таск FreeRTOS, период 2 мс)
...
**Калибровка IMU**:
- **При старте (автоматически)**: если машина неподвижна (variance < threshold), вычислить gyro bias за ~1-2 секунды (500-1000 семплов). Если motion detected — использовать сохранённые данные из NVS
- **По команде (ручная)**: оператор ставит машину на ровную поверхность, запускает полную калибровку через WebSocket — гироскоп bias + акселерометр offset/scale
...
**Критерии готовности**:
- IMU читается стабильно на 500 Гц без пропусков на Core 1
- Калибровка гироскопа выполняется автоматически при старте (если машина неподвижна)
- Ручная калибровка запускается через WebSocket API и завершается за <5 секунд
- Калибровочные данные сохраняются в NVS и корректно восстанавливаются при перезагрузке
- Madgwick даёт стабильную ориентацию (pitch/roll без дрейфа) на откалиброванных данных
- Данные фильтрации и статус калибровки доступны в телеметрии через WebSocket
- Режим стабилизации можно включать/выключать через WebSocket API
- Без стабилизации система работает как раньше
- Control loop на Core 1 не влияет на работу Wi-Fi/WebSocket на Core 0
