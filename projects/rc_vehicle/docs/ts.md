...
- Оптический датчик “скорости по земле” (optical flow), например **PMW3901** или аналог:
  - даёт оценку **скорости/смещения относительно поверхности**, полезно для одометрии и контроля **пробуксовки** (в отличие от Холла по колёсам);
  - интерфейс обычно **SPI**, питание/логика **3.3V**;
  - для корректного масштаба скорости желательно знать высоту до поверхности: либо фиксированная геометрия, либо добавить **ToF вниз** (например, VL53L1X/VL53L0X) и использовать её для пересчёта.
